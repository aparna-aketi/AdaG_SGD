...
from gossip_choco import RingGraph, GridGraph, FullGraph, PetersenGraph, DyckGraph
...
parser.add_argument('--normtype',   default='evonorm', help = 'batchnorm or groupnorm or evonorm' )
parser.add_argument('-b', '--batch-size', default=512, type=int,  metavar='N', help='mini-batch size (default: 128)')
parser.add_argument('--lr', '--learning-rate', default=0.1, type=float,     metavar='LR', help='initial learning rate')
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',     help='momentum')
parser.add_argument('--nesterov', action='store_true', )
parser.add_argument('--ada_gamma', action='store_true', )
parser.add_argument('--qgm', action='store_true', help="enables quasi global momentum" )
...
parser.add_argument('--partition_type',    help='The type of data distribution - random or non_iid_dirchilet',   default="random", type=str)
parser.add_argument('--alpha', default=10.0, type=float,     help='0.01 = most heterogeneous and 10 = most iid')
...
def run(rank, size):
    ...
    for epoch in range(0, args.epochs):  
        ...
        model.block()
        dt= train(train_loader, model, criterion, optimizer, epoch, args.batch_size, optimizer.param_groups[0]['lr'], device)
        #print((dt*200)/1.0e9)
        data_transferred += dt
        lr_scheduler.step()
        prec1 = validate(val_loader, model, criterion, device, epoch)
    ...
    prec1_final = validate(val_loader, model, criterion, device, epoch)
...
def train(train_loader, model, criterion, optimizer, epoch, batch_size, lr, device):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        optimizer.step()
        optimizer.zero_grad()
        _, amt_data_transfer = model.transfer_params(optimizer, lr=lr)
...
if __name__ == '__main__':
    ...
    excel_data = {
        ...
        "gamma" : args.gamma,
        "compress fn": args.fn, 
        "compression ratio" : args.k,
        "bit precision" : args.level,
        "learning rate": args.lr,
        "momentum":args.momentum,
        "nesterov": args.nesterov,
        "qgm": args.qgm,
        "weight decay"      : args.weight_decay,
        "adaptive_gamma" : args.ada_gamma,
