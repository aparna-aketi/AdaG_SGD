...
class Gossiper(object):
    ...
    def __init__(self, msg, graph, device=None, logger=None,
                 rank=None, world_size=None, mixing=None):
        """
...
class SGD_DS(Gossiper):
    ...
    def mix(self, out_msg, ref_msg, uncompress=False, shapes =None):
        ...
        if uncompress:
            ...
            mixed_out_msgs = self.mix_out_msg_(values)
        else:
        ...
        for in_edge in self.in_edges:
            ...
            if uncompress:
                received_msg = flatten_tensors(unsparsify_layerwise(placeholder.narrow(0, 0, len(placeholder)), shapes, ref_msg))
                self.in_msg_buffer.add_(received_msg)
            else:
